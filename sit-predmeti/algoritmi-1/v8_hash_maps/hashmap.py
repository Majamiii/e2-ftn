...
class HashMap(object):
    ...
    def _hash(self, x):
        ...
        """
        hashed_value = (hash(x)*self._a + self._b) % self.prime     # ugradjena pajtonova metoda za heširanje
        #   ovo gore je MAD hashiranje, lepo rasporedjuje vrednosti  -  multiply, add, divide
        #   malo duplikata
...
class LinearHashMap(HashMap):
    ...
    def _find_bucket(self, bucket_index, key, check_deleted=False):
        ...
        """
        # sve ostale metode se oslanjaju na ovu

        available_slot = None                               # prvo slobodno mesto gde možemo da stavimo naš element
        while True:
            if self._is_available(bucket_index):
                if available_slot is None:
                    available_slot = bucket_index           # ako postoji element sa tim ključem apdejtujemo ga sa novoprosleđenom vrednošću

                if self._data[bucket_index] is None:
                    return False, available_slot            # nema ni marker -> neuspešna pretraga, element ne postoji
            ...
            bucket_index = (bucket_index+1) % len(self._data)       # ovako je implementirana cirkluarnost
