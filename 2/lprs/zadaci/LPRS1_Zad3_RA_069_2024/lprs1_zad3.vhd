...
constant cSECOND			: std_logic_vector(23 downto 0):="000000000000000000000110";	-- zbog simulacije stavljeno da je 1 sekunda=6 taktova.
-- za realan sistem iskoristiti zakomentarisanu vrednost konstante cSECOND iz 43. linije
...
constant cDIS_MAX			: std_logic_vector(14 downto 0) := "000000000000010";-- zbog simulacije stavljeno da se menjanje displeja vrsi na svaka 2 takta.
-- za realan sistem iskoristiti zakomentarisanu vrednost konstante cDIS_MAX iz 63. linije
...
begin
	...
	process (iCLK,iRST,iRUN) begin
		...
		-- i jednom kad se aktivira ne menja se sve do reseta
			if (iRUN='1')then
				sPREV_STATE<='1';
			end if;
	...
	-- dozvolice se da se desetice smanje samo ako su jedinice dosle do 0 u proslom taktu i sad su vec presle na 9
	-- npr 10 - 09
	...
	-- brojac po modulu 3 -> 3 puta broji po pola sekunde
	process(iCLK,iRST)begin
		if(iRST='1')then
	...
	-- kada brojac izbroji do 2 automat prelazi u sledece stanje (0->1->2 jer je po modulu 3)
	...
	-- SIGNALI koji opisuju kada se prikazuju brojevi na 7seg i sta se prikazuje.
	-- ako nije u adekvatnom stanju salje sve jedinice, sto ce
	-- dovesti do toga da se na displeju nista ne prikazuje
	...
	-- dekoderi za 7seg displej
	-- sve jedinice=default -> displej je iskljucen kada taj signal nije aktivan
	
	-- zelene desetice - mogu biti 0 ili 1. ako nije nista od toga onda ga iskljuci.
	sDISPLAY_3 <= "0000001" when sGREEN_TENS_DISPLAY="0000" else
					"1001111" when sGREEN_TENS_DISPLAY="0001" else
					"1111111";
					
	-- zelene jedinice. ako nije 0-9 onda ga iskljuci.				
	...
	-- crvene desetice - mogu biti 0,1 ili 2.  ako nije nista od toga onda ga iskljuci.
	sDISPLAY_1 <= "0000001" when sRED_TENS_DISPLAY="0000" else
