...
struct Odsecak
{
    ...
    Odsecak(int* mm, int nn, bool slobodno) : m(mm), n(nn), free(slobodno) {}
};
...
private:
    ...
    int kapacitet;
    mutex mtx;
    condition_variable cv_alociranje;
    condition_variable cv_komprimovanje;
    bool zatrazenoSazimanje;
    bool kraj;
...
public:
    Memorija(int kapacitet_p, Dijagnostika &dd) : d(dd)
    {
        kapacitet = kapacitet_p;
        p_memorija = new int[kapacitet_p];
        Odsecak o(p_memorija,kapacitet,true);
        evidencija.push_back(o);
        zatrazenoSazimanje = false;
        kraj = false;
    }
    ~Memorija()
    {
    ...
    {
        // Napisati thread-safe, sinhronizovan kod koji eliminse sansu stetnih preplitanja
        ...
        {
            d.stampajPoruku(tag, "cekam na zauzece memorije");
            cv_alociranje.wait(ul);
        }
        d.stampajPoruku(tag, "zauzeo memoriju");
        return it->m;
    }
    ...
    void dealociraj(int *p, const char *tag = "Unknown")
    {
        ...
        for (auto it = evidencija.begin(); it != evidencija.end(); it++)
        {
            if (it->m == p)
            {
                d.stampajPoruku(tag, "oslobodio memoriju");
                it->free = true;
            }
        }
        cv_alociranje.notify_all();
    }
    void kompaktiraj()
    {
        ...
        while (true)
        {
            ...
            unique_lock<mutex> ul(mtx);
            while (!zatrazenoSazimanje && !kraj)
            {
                d.stampajPoruku("kompakcija_nit", "cekam na kompakciju ili kraj");
                cv_komprimovanje.wait(ul);
            }
            ...
            for (auto it = evidencija.begin(); it != evidencija.end(); it++)
            {
                if (it->free)
                {
                    auto it2 = it;
                    it2++;
                    while (it2->free && it2 != evidencija.end())
                    {
                        // Produzi
                        it->n += it2->n;
                        // Obrisi
                        d.stampajPoruku("kompakcija_nit", "brisem nadjeni odsecak\n");
                        evidencija.erase(it2);
                        d.stampajPoruku("kompakcija_nit", "obrisao nadjeni odsecak\n");
                        it2 = ++it;
                        it--;
                    }
                }
            }
            d.stampajPoruku("kompakcija_nit", "zavrsio sa kompakcijom, obavestavam ostale niti");
            cv_alociranje.notify_all();
        }
    }
};
...
void racunaj(int od, const int N, int *gde)
{
    // prostih brojeva i smestiti rezultate u niz na koji pokazuje 'gde.'
    // Efikasnost algoritma se ne ocenjuje.
    const int NAJMANI_DELILAC = 2;
    int brojNadjenih = 0;
    while (brojNadjenih < N)
    {
        bool jesteProst = true;
        for (int delilac = NAJMANI_DELILAC; delilac < od; delilac++)
        {
            if (od % delilac == 0)
            {
                jesteProst = false;
...
void testirajA(Dijagnostika &d)
{
    ...
    int *p[8];
    thread niti[8];
    const int OD_PO_NITI = 2000;
    const int PROSTIH_BROJEVA_PO_NITI = 128;
    for (int i = 0; i < 8; i++)
    {
        p[i] = mem + (i * 128);
        ...
        niti[i] = thread{racunaj, i * OD_PO_NITI, PROSTIH_BROJEVA_PO_NITI, p[i]};
        // cout << "Startovana " << i << "ta nit" << endl;
    }
    for (int i = 0; i < 8; i++)
    {
        niti[i].join();
...
void testirajB(Memorija &mem)
{
    ...
    for (int i = 0; i < 10; i++)
    {
        ...
        int n = rand() % 6 + 1;
        //  Pokrenuti nit koja ima ime smesteno u tags[i] i koja radi sa memorijom mem i racuna
        // n blokova prostih brojeva kroz calculationThread funkciju
        niti[i] = thread{calculationThread, tags[i], ref(mem), n};
    }
    //  join-ovati sve pokrenute niti.
    for (auto &t : niti)
    {
        t.join();
    }
...
void testirajC(Memorija &mem)
{
    ...
    thread kompakcijaDemon{compactionDeamon, ref(mem)};
    kompakcijaDemon.detach();

    thread timerNit{timerThread, ref(mem), ref(runTimer)};
    timerNit.detach();
