...
bool passI()
{
	...
	*******************************************************************************************/
	...
	for (sourceListIter = sourceList->begin(); sourceListIter != sourceList->end(); sourceListIter++)
	{
		...
		// TODO: Extract symbol from the line and add it to the symbol table if it does not exist.
		//       Report an error if the symbol is already defined.
		if (getSymbol(symbol, sourceLine)) {
			if (!symbolExists(symbol)) {
				pushSymbol(symbol, memLocCounter, section);
			}
			else {
				addError(lineNum, sourceLine, "Error: Symbol already exists.\n");
			}
		}

		// TODO: Detect executable instruction and increment memLocCounter.
		//       Skip further processing since instruction and directive cannot be in the same line.
