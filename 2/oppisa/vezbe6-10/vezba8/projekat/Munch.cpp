...
Reg munchExp(Expression* expression)
{
	switch (expression->getType())
	{
		case Expression::MEM_EXP:
		{
			...
			Expression* expression = memExp->getExp();
			...
			if (expression->getType() == E_Binop::BINOP_EXP) {

				E_Binop* binop = static_cast<E_Binop*>(expression);

				if (binop->getOperType() == E_Binop::PLUS_OP) {

					leftExp = binop->getLeftExp();
					rightExp = binop->getRightExp();

					if (rightExp->getType() == Expression::CONST_EXP)
					{

						E_Const* constExp = static_cast<E_Const*>(rightExp);
						instructionString = "lw `d," + toString(constExp->getValue()) + "(`s)";
						srcExp1 = leftExp;
			...
			else if (expression->getType() == Expression::CONST_EXP) {
				E_Const* constExp = static_cast<E_Const*>(expression);
				instructionString = "lw `d," + toString(constExp->getValue()) + "(`s)";

				Instruction* instr = new Instruction(instructionString);
				instr->getDst().push_back(dstReg);
				Reg srcReg = munchExp(constExp);
				instr->getSrc().push_back(srcReg);
				emit(instr);
				return dstReg;
			}
			else {
				instructionString = "lw `d,0(`s)";
				Instruction* instr = new Instruction(instructionString);
				Reg srcReg = munchExp(expression);
				instr->getDst().push_back(dstReg);
				instr->getSrc().push_back(srcReg);
				emit(instr);
				return dstReg;
		...
		case Expression::BINOP_EXP:
		{
			...
			if (binopExp->getOperType() == E_Binop::PLUS_OP)
			{
				if (rightExp->getType() == Expression::CONST_EXP)
				{
					...
					E_Const* constExp = static_cast<E_Const*>(rightExp);;
					instructionString = "addi `d,`s,const";
					srcExp1 = leftExp;
