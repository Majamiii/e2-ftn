...
public:
    ...
    bool preveziTuru()
    {
        ...
        // ako je broj sedista zicare veci ili jednak broju putnika,
        // to znaci da se ukupan broj voznji povecava za onoliko koliko je putnika cekalo.
        // Nakon prevozenja, lista putnika koji cekaju ostaje prazna
        if(BROJ_SEDISTA>=putnici.size()){
            ukupnoVoznji += putnici.size();
            putnici.clear();
        }
        // ako je broj sedista zicare manji od broja putnika, to znaci da ne staju svi u zicaru
